...
**핵심 테제:**
> GENIUS Act(2025-07 시행)·MiCA(2024-06 시행)로 스테이블코인 B2B 결제 인프라가 제도권에 편입되고 있다. B2B 스테이블코인 payments는 이미 연 $36B 규모로 검증됐으며(Artemis, 2025-05), Visa·Circle 등 기관이 본격 참여 중이다. 현재 거시 레짐은 **Goldilocks with sticky inflation** — 성장은 유지되지만 금리 인하는 제한적이어서, 준비금 이자 의존형보다 **결제 수수료 SaaS 모델**이 구조적으로 유리하다. 다날의 SC 정산 SaaS·x402 전략은 이 전환의 최적 타이밍에 위치하며, 이를 가속할 투자·파트너십 대상 선별이 현재 가장 중요한 의사결정이다.
...
| **Ripple** | 크로스보더 정산·스테이블코인 플랫폼 인프라 | 관심 → IPO 후 검토 상향 | ★★☆ |
...
| USD/KRW | **1,445.97** | 달러 강세 — 크립토 자금흐름 참고 | FRED, 2026-03-06 |
...
**핵심 인사이트:** 금리 하락은 **준비금 이자 의존형 모델**(Tether, Circle)에는 역풍이지만, **결제 수수료 기반 SaaS 모델**(다날 방향)에는 중립~긍정이다. 이것이 다날이 지금 SC 정산 SaaS 모델로 전환해야 하는 거시적 근거다.
...
| **GENIUS Act 시행 세부규정** | 미국 | ⚠️ **시행 세부규정 확정 대기** | 발행 주체 요건·감독 구조 최종 가이던스 대기 |
...
**GENIUS Act 시행 세부규정 현황:**
- 2026년 3월 기준 시행 세부규정(implementing rules) 확정 대기
- OCC·Fed·FDIC 공동 가이던스 발표 예정 — 발행 주체 자본 요건·감독 체계 최종 확정 대기
- SEC 증권성 판단 기준 진화 중 — 적격 스테이블코인 면제 범위 명확화 필요

**결론:** 미국은 규제 완료 → 기관 채택 가속화 단계, EU는 라이선싱·집행 본격화 단계, 미국은 GENIUS Act 시행 세부규정 확정 대기 단계. 규제 성숙도 차이가 **다날의 기회이자 리스크**다. 세부규정 확정 전에 글로벌 파트너와의 기술 협력을 선점하는 것이 최적 전략이다.
...
- 🔴 **테제 무효 신호**: GENIUS Act 시행 후 주요 스테이블코인 발행사 규제 위반·제도 붕괴 + GENIUS Act 시행 세부규정 2년 이상 지연 → 규제 명확화 논거 붕괴
- 🔴 **전략 재검토 신호**: Fed 긴급 인상 재개(FF ≥ 5%) → Stagflation 전환 → 기업 디지털자산 투자 위축
- 🟡 **속도 조정 신호**: 슈퍼블록 x402 PoC 1년 내 상용화 실패 → 기술 파트너십 타당성 재평가
- 🟢 **테제 강화 신호**: USDC 시총 $100B 돌파 + 글로벌 스테이블코인 B2B 결제 $100B/년 돌파
...
| 축 | 사업 | 현황 |
|---|---|---|
| **스테이블코인 정산 SaaS** | 스테이블코인 발행·정산 플랫폼 | **KBW 2025 실제 시연 완료** (2025-09-23) |
...
> 참가자가 실제로 Fiat을 스테이블코인으로 mint하고 행사장 내 가맹점 바코드 결제 수행. "전략" 단계를 넘어 **실물 결제 PoC 검증** 완료.
> — 뉴시스, 2025-09-24
...
3. **국내 PG 시장 성숙**: 국내 PG 시장 점유율 경쟁은 가격 경쟁 위주. 마진 방어보다 새로운 수익원 발굴이 전략적 과제. SC 정산 SaaS는 기존 가맹점망(다날 PG)에 온체인 정산 레이어를 얹는 구조 — 신규 투자 대비 수익 효율이 높다.
...
- 다날 PG 가맹점망 활용 → 가맹점의 5~10% SC 정산 SaaS 전환
...
2. **전략 수렴**: 다날 3축(SC 정산 SaaS·x402·PCI) 모두 이 섹터와 직결
...
| 기준 | 가중치 | 측정 방법 |
|---|---|---|
| 다날 전략 적합도 | 40% | SC 정산 SaaS·x402·PCI 3축 중 보완하는 축의 수 |
| 기술 보완성 | 30% | 다날이 내재화하지 못한 기술 보유 여부 |
| 규제 대응력 | 20% | GENIUS Act·MiCA·한국법 준수 체계 |
| 협력 현실성 | 10% | 기존 관계·이해 충돌 수준 |

### 4.1-b 스테이블코인 경쟁 지형 (Tiger Research, 2025)

현재 screen_stablecoin 분석은 글로벌 경쟁사 중심으로 작성되어 있으나, 아시아 시장에서의 스테이블코인 추진 기업들도 참고할 필요가 있다:

| 플레이어 | 체인 | 현황 | 다날 대비 |
|---------|------|------|---------|
| BDACS + 우리은행 | Avalanche | KRW1 테스트넷, Circle ARC 네트워크 연계 | 은행 백킹 → 신뢰도 高, 속도 低 |
| 네이버-두나무 | 카이아(Kaia) | 부산 동백전 연계, 네이버파이낸셜 지갑 | 대형 플랫폼 + 지자체 채널 |
| 카카오뱅크 | 미정 | 2025년 중 스테이블코인 목표 공표 | 2,400만 사용자 기반 강점 |
| 프랙스-IQ | 미정 | 스테이블코인 개발 중 | 알고리즘 스테이블코인 경험 |
...
**규제 구도:** GENIUS Act 하에서 연방 감독(>$10B 발행사) vs 주 감독(소규모 발행사) 이원 체계. OCC·Fed·주 규제 당국 간 관할 정리 진행 중 — 최종 가이던스에 따라 비은행 발행사(다날 포함)의 포지셔닝이 유리/불리로 변동 가능.
...
| **Ripple** | ✅ 선택 | XRPL = 다날 스테이블코인 플랫폼 기반 인프라, RLUSD+ODL 수직 통합 |
...
**다날과의 연결:**
- CCTP를 통해 SC 정산 SaaS의 글로벌 USDC 정산 레일 활용 가능
...
**포지션:** XRPL 기반 크로스보더 결제 + RLUSD 스테이블코인 + ODL(On-Demand Liquidity) 수직 통합. 다날 스테이블코인 플랫폼의 기반 인프라.

**다날과의 연결:**
- 다날 스테이블코인 플랫폼(페이코인 기반)은 XRPL 위에서 운영 → Ripple은 기술 인프라 파트너
- SEC 소송 종결 + XRP ETF 가능성 → 기관 자금 유입 시 Ripple 기업가치 재평가
- RLUSD-다날 플랫폼 공존 시나리오: RLUSD는 글로벌 정산, 다날 스테이블코인은 결제 인프라로 역할 분리
...
**다날 투자 배경 분석:**
- 다날의 SC 정산 SaaS에 스테이블코인 정산 레이어(Over Protocol) 제공
...
| 기업 | 권고 | 액션 | 이유 |
|---|---|---|---|
| **Circle** | 파트너십 검토 | CCTP API 연동 PoC 제안 | IPO 후 공식 파트너십 채널 형성 + USDC 네트워크 편입이 SC 정산 SaaS 글로벌화 가장 빠른 경로 |
| **Ripple** | 관심 유지 | IPO 모니터링 + 스테이블코인 플랫폼-XRPL 연동 심화 | 이미 기반 인프라로 연결됨 — 전략적 심화는 IPO 이후 재무 투명성 확보 후 결정 |
| **슈퍼블록** | 후속 관리 | PoC 마일스톤 KPI 설정 + 상용화 시 2차 투자 검토 | 기투자 완료 — 의사결정 이슈는 "얼마나 더"이며, 이는 x402·SC 정산 PoC 성과에 달려 있음 |
...
- [ ] 다날 스테이블코인 플랫폼 메인넷 거래량 기반 XRPL 트랜잭션 비용 추이
...
1. **단기 (1개월)**: Circle CCTP 개발자 문서 분석 → 다날 SC 정산 SaaS 연동 기술 타당성 검토
...
**완료:**
- `outputs/csv/kwrw_stablecoin_scenario_20260308.csv` ✅ — SC 정산 SaaS 수익 시나리오 27개 (3규제×3전환율×3take rate)
